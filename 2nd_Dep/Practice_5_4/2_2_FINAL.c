...
int leer_p_box(P_BOX P, P_BOX P_inv, const char *filename) {
    ...
    char header[8];  // suficiente para "P_inv:" + '\0'
...
unsigned char permutacion_p_byte(unsigned char bloque8, const P_BOX P) {
    ...
    for (int i = 0; i < 8; i++) {
        unsigned char bit = (bloque8 >> (7 - i)) & 0x01;
        
        int new_pos_idx = P[7-i];
        
        resultado |= (bit << new_pos_idx);
    }
    return resultado;
}
...
int main() {
    ...
    printf("1) Generar y guardar llave K, S-box S, y Permutacion P/P^-1\n");
    printf("2) Cifrar un caracter con el Algoritmo 1 (con P)\n");
    ...
    if (opcion == 1) {
        ...
        printf("Nombre de archivo para guardar la P-box P y P^-1: ");
        scanf("%127s", archivo_pbox);
        ...
        printf("P-box/P^-1 guardada en %s\n", archivo_pbox);
        ...
        printf("Nombre de archivo donde esta la P-box P: ");
        scanf("%127s", archivo_pbox);
